...
(async () => {
  ...
  try {
    ...
    await page.evaluate(() => localStorage.setItem('apty.malayalam.preview.v1', JSON.stringify({ episode: 2, step: 1 })));
    await page.reload({ waitUntil: 'networkidle' });
    await page.getByRole('link', { name: 'തുടങ്ങാം', exact: true }).waitFor();
    assert.equal(await page.locator('.ml-next-card .ml-btn').count(), 1);
    assert.equal(await page.locator('.ml-ready-lesson').count(), 0);
    assert.equal(await page.locator('.ml-lesson-cards').count(), 0);
    ...
    await page.getByRole('link', { name: 'തുടരാം', exact: true }).waitFor();
    ...
    assert.equal(await page.evaluate(() => document.activeElement.getAttribute('aria-label')), 'Open preview 1: തറ');
    await page.reload({ waitUntil: 'networkidle' });
    await page.getByRole('button', { name: /വാക്കുകൾ/ }).click();
    await page.getByRole('button', { name: 'Open preview 1: തറ', exact: true }).click();
    assert.equal(await page.locator('.ml-model-letter').innerText(), 'ത');
    ...
    await page.getByRole('button', { name: /എന്റെ വഴി/ }).click();
    const responsive = [];
    ...
    await blockedPage.getByRole('button', { name: /വാക്കുകൾ/ }).click();
    await blockedPage.getByRole('button', { name: 'Open preview 1: തറ', exact: true }).click();
